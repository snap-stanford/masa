...
def computeClusterBIC(K,T, clustered_points, inverse_covariances, empirical_covariances, motifs):
    ...
    non_zero_params = 0
    motifCounter = -1
    for m, instances in motifs.items():
        clusterParams[motifCounter] = np.mean([clusterParams[c] for c in m])
        for start, end in instances:
            clustered_points[start:end+1] = [motifCounter for _ in range(start, end+1)]
        motifCounter -= 1
